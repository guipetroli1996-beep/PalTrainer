...
| 3 | Aim+LMB filler + follow-only standby + Aim+1/2/3 skills | Working (LogicMod optional) |
| 4 | Multiplayer | Later |
...
## Phase 3 standby — LogicMod (TrainerCombatBP)

Lua alone cannot reliably stop `PalAIActionCombat_Standard`. Standby enforcement is stronger with a cooked **LogicMod**.

### Prerequisites

| Step | Notes |
|------|--------|
| [Palworld Modding Kit](https://pwmodding.wiki/docs/developers/palworld-modding-kit/prerequisites) | Clone locally (path is yours) |
...
| Deploy `.pak` to `Paks/LogicMods` | `scripts\deploy-logicmod.ps1` |
...
5. Package Windows → copy `pakchunk7-Windows.pak` →  
...
Until the `.pak` exists, Lua uses a **NotCombat order fallback**.
...
4. Enter a world, throw a Pal → standby / follow-only.
5. Aim+LMB on a hostile → Pal filler attack, then standby.
6. In combat, swap/recall should hit summon lock CD; sphere throws use sphere CD.
...
5. After LogicMod cook: run `scripts\deploy-logicmod.ps1`, then full relaunch.
